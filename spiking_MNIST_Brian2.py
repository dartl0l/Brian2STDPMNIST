...
MNIST_data_path = '/s/ls4/users/dartl0l/spikes/genetics/mnist_diehl/data/'
...
def get_matrix_from_file(fileName, ending, n_input, n_e, n_i):
    offset = len(ending) + 4
...
def get_2d_input_weights(connections, n_input, n_e):
    name = 'XeAe'
...
def plot_2d_input_weights(connections, n_input, n_e, fig_num, wmax_ee):
    name = 'XeAe'
...
if test_mode:
    ...
    num_examples = train_data_size * 3
...
if num_examples <= test_data_size:
    update_interval = num_examples
...
for subgroup_n, name in enumerate(population_names):
    ...
    for conn_type in recurrent_conn_names:
        ...
        weightMatrix = get_matrix_from_file(weight_path + '../random/' + connName + ending + '.npy', ending, n_input, n_e, n_i)
        model = 'w : 1'
...
for name in input_connection_names:
    ...
    for connType in input_conn_names:
        ...
        weightMatrix = get_matrix_from_file(weight_path + connName + ending + '.npy', ending, n_input, n_e, n_i)
        model = 'w : 1'
...
if not test_mode:
    input_weight_monitor, fig_weights = plot_2d_input_weights(connections, n_input, n_e, fig_num, wmax_ee)
    fig_num += 1
...
while j < int(num_examples):
    if test_mode:
        if use_testing_set:
            spike_rates = testing['x'][j%test_data_size,:,:].reshape((n_input)) / 8. *  input_intensity
        else:
            spike_rates = training['x'][j%train_data_size,:,:].reshape((n_input)) / 8. *  input_intensity
    else:
        normalize_weights(n_e)
        spike_rates = training['x'][j%train_data_size,:,:].reshape((n_input)) / 8. *  input_intensity
...
#     print('run number:', j+1, 'of', int(num_examples))
    ...
    if np.sum(current_spike_count) < 5:
        ...
        if test_mode and use_testing_set:
            input_numbers[j] = testing['y'][j%test_data_size][0]
        else:
            input_numbers[j] = training['y'][j%train_data_size][0]
...
plot_2d_input_weights(connections, n_input, n_e, fig_num, wmax_ee)
